...
- "Monorepo with shared components in /lib — never duplicate shared utilities."
- "All API routes include error handling middleware."
- "State management uses [X]. Do not introduce alternatives."
-->
...
- "Prefer built-in APIs over npm packages when equivalent."
-->

### Testing
<!-- Examples — uncomment and customize:
- "Test command: `npm test` (unit), `npm run test:integration` (integration)."
- "Every new function requires a test in __tests__/"
- "Integration tests clean up their own test data."
-->

### Boundaries
<!-- Examples — uncomment and customize:
- "Never modify files in /shared without checking all consuming services."
